...
- **Fe8Processor**：调用 fe8.cn 提供的 GPT 系列模型（gpt-3.5-turbo、gpt-4-turbo），用于问题重写/分类和最终回答生成
...
- **问题重写与分类**：调用 Fe8Processor（gpt-3.5-turbo）进行问题分类和重写，输出包含 `completed_question`（补全后的完整问题）、`rewritten_query`（检索用查询）、`doc_type`（文档类型）
- **追问检索增强**：调用 `expand_followup_query` 从前一轮问题中提取财务术语，合并到检索查询
- **历史摘要注入**：调用 `summarize_history_for_rewrite` 将对话历史格式化为问题重写的上下文
- **回答分段**：Prompt 要求 LM 按要点分段输出 final_answer
...
8. **问题重写与分类**：`pipeline.py` → 调用 `Fe8Processor`（gpt-3.5-turbo），注入历史上下文，输出 `completed_question`、`rewritten_query`、`doc_type`
9. **追问检索增强**：`questions_processing.py` → `expand_followup_query` 从前一轮提取财务术语，合并到检索查询
10. **检索**：`retrieval.py` → BM25 / Vector / Hybrid / HybridBM25Vector 四种检索器
11. **重排**：`reranking.py` → DashScope gte-rerank-v2 精排
12. **LLM交互**：`api_requests.py` → `Fe8Processor`（gpt-4-turbo）生成分段回答
13. **主流程调度**：`pipeline.py` → 统一配置与流程编排，管理历史注入和回退逻辑
