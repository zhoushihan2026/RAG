            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        raise RuntimeError(
                            f"结构化请求三次均失败。beta.parse: {e1}。json_object: {e2}。plain: {e3}"
                        )
            ...
            if is_structured and response_format is not None:
                try:
                    ...
                    try:
                        ...
                        # 将 Unicode 减号替换为 ASCII 连字符，避免 Windows gbk 编码错误
                        reparsed_safe = str(reparsed).replace('\u2212', '-')
                        print(f"Reparsed response: {reparsed_safe}")
                        return content
    ...
    def _reparse_response(self, response, response_format):
        ...
        try:
            ...
            try:
                ...
                print(f"Reparse failed with error: {reparse_err}")
                # 将 Unicode 减号替换为 ASCII 连字符，避免 Windows gbk 编码错误
                reparsed_response_safe = str(reparsed_response).replace('\u2212', '-')
                print(f"Reparsed response: {reparsed_response_safe}")
                return response
        except Exception as e:
...
class APIProcessor:
    ...
    def _build_rag_context_prompts(self, schema):
        ...
        elif schema == "fact_extraction":
            system_prompt = (prompts.AnswerWithRAGContextFactPrompt.system_prompt_with_schema
                            if use_schema_prompt else prompts.AnswerWithRAGContextFactPrompt.system_prompt)
            response_format = prompts.AnswerWithRAGContextFactPrompt.AnswerSchema
            user_prompt = prompts.AnswerWithRAGContextFactPrompt.user_prompt
        elif schema == "analysis_explanation":
            system_prompt = (prompts.AnswerWithRAGContextAnalysisPrompt.system_prompt_with_schema
                            if use_schema_prompt else prompts.AnswerWithRAGContextAnalysisPrompt.system_prompt)
            response_format = prompts.AnswerWithRAGContextAnalysisPrompt.AnswerSchema
            user_prompt = prompts.AnswerWithRAGContextAnalysisPrompt.user_prompt
        elif schema == "prediction_judgment":
            system_prompt = (prompts.AnswerWithRAGContextPredictionPrompt.system_prompt_with_schema
                            if use_schema_prompt else prompts.AnswerWithRAGContextPredictionPrompt.system_prompt)
            response_format = prompts.AnswerWithRAGContextPredictionPrompt.AnswerSchema
            user_prompt = prompts.AnswerWithRAGContextPredictionPrompt.user_prompt
        else:
...
class BaseDashscopeProcessor:
    ...
    def _get_openai_client(self):
        """获取DashScope OpenAI兼容客户端（用于多模态模型）"""
    ...
    def _is_multimodal_model(self, model):
        """判断模型是否需要使用OpenAI兼容接口"""
        if model is None:
            return False
        return model in self.MULTIMODAL_MODELS

    def _get_async_openai_client(self):
        """获取DashScope OpenAI兼容异步客户端（用于并发调用）"""
    ...
    def send_message(
        ...
        **kwargs
        ...
        if self._is_multimodal_model(model):
            return self._send_message_openai_compat(model, messages, temperature)
        ...
        print('model=', model)
        # 清理所有可能导致Windows gbk编码错误的特殊字符
        def clean_unicode_for_print(text):
            """替换所有可能导致Windows gbk编码错误的Unicode字符"""
            if not text:
                return text
            replacements = {
                '\u2705': '[OK]',
        ...
        response_str = clean_unicode_for_print(str(response))
        ...
        # 使用相同的清理函数处理content
        content_str_safe = clean_unicode_for_print(str(content))
        print('content=', content_str_safe)
        ...
        try:
            ...
            # 如果不是有效的JSON，返回基本格式
            # 将 Unicode 减号替换为 ASCII 连字符，避免 Windows gbk 编码错误
            content_safe = str(content).replace('\u2212', '-')
            print(f"Content is not valid JSON, returning basic format: {content_safe}")
            return {"final_answer": content, "step_by_step_analysis": "", "reasoning_summary": "", "relevant_pages": []}
    ...
    def _send_message_openai_compat(self, model, messages, temperature):
        ...
        try:
            ...
            content = completion.choices[0].message.content
            print(f'[OpenAI兼容] model={model}, content长度={len(content) if content else 0}')
    ...
    def send_message_stream(self, model="qwen-turbo-latest", temperature=0.1,
        ...
        for resp in responses:
            if hasattr(resp, 'output') and hasattr(resp.output, 'choices') and resp.output.choices:
                delta = resp.output.choices[0].message.content
                if delta:
                    full_content += delta
                    yield delta
            elif hasattr(resp, 'status_code') and resp.status_code != 200:
                error_msg = f"[API错误] {resp.code if hasattr(resp, 'code') else ''}: {resp.message if hasattr(resp, 'message') else str(resp)}"
                yield error_msg
                return
    ...
    def _send_message_stream_openai_compat(self, model, messages, temperature):
        """通过DashScope OpenAI兼容接口流式发送消息（用于多模态模型）"""
        client = self._get_openai_client()
        try:
