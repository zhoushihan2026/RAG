...
最终答案为完整、清晰的文本，需基于上下文内容作答。
**必须分段输出**，用空行分隔不同要点，提升可读性：
- 不同指标或数据应分段呈现
- 核心结论与补充说明应分段
- 如有多个要点，每个要点单独一段
...
```
{
  ...
  "final_answer": "万科企业股份有限公司2022年主营业务包括：\n\n房地产开发：住宅及商业地产开发与运营，是公司核心业务。\n\n物业服务：物业管理及相关增值服务。\n\n租赁住房：长租公寓运营。\n\n物流仓储：物流地产开发与运营。"
}
```
'''
...
```
{
  ...
  "relevant_pages": [5],
  "final_answer": "2024年全年营业收入约577.96亿元（57,795,570千元）。\n\n同比增长约27.73%。\n\n毛利率约18.59%。"
}
```
'''
...
- **重要**：对于"原因"、"为什么"类问题，即使上下文没有直接写明原因，也应从相关数据中推导：
  - 例如问"净利润下降原因"，上下文可能只列出了"折旧增加XX亿、资产减值XX亿"，你需要从这些数据推导出"折旧和减值是利润下降的主要原因"
  - 推导时必须引用具体数据作为依据，不能凭空推测
  - 如果数据不足以推导原因，说明已知的关联数据并指出需要更多信息
"""
...
```
{
  ...
  "relevant_pages": [5, 8],
  "final_answer": "2024年归母净利润约36.99亿元，同比下降23.3%。\n\n下降的主要原因：\n\n1）折旧和摊销大幅增加：产能扩张导致固定资产增长，折旧压力加大。\n\n2）资产减值损失。\n\n3）投资收益波动。\n\n此外，营业成本增幅（33.1%）显著高于营业收入增幅（27.7%），也是利润承压的底层因素。"
}
```
'''
...
""")
        final_answer: str = Field(description="""
最终答案为完整、清晰的文本，以预测/判断性内容为主。
**必须分段输出**，用空行分隔不同要点，提升可读性：
- 预测数据必须标注来源（券商名称），不同来源分段呈现
- 评级和目标价必须标注券商
- 管理层观点标注来源
- 如有多个来源，列出差异
- 区分共识和分歧
...
HISTORY_REWRITE_SYSTEM_PROMPT = """你是一个多轮对话的问题补全与关键词扩展模块。你的任务有两个：
1. 根据对话历史，将用户的当前问题补全为一个独立、完整、自洽的问题
2. 对补全后的问题进行关键词扩展（同义词、相关术语），以提升检索召回率

## 补全规则（按优先级排列）：

### 1. 代词替换
将代词替换为对话历史中实际指代的实体：
- "它"/"其"/"该公司"/"该企业" → 替换为历史中提到的公司全称
- "他"/"她" → 替换为历史中提到的人名
- "这个指标"/"这个数据" → 替换为历史中讨论的具体指标名称

### 2. 省略主语补全
当用户省略了主语时，从对话历史中继承：
- "毛利率呢？" → "XX公司2024年全年的毛利率是多少？"
- "同比增长了多少？" → "XX公司2024年营业收入同比增长了多少？"
- "研发费用是多少？" → "XX公司2024年的研发费用是多少？"

### 3. 时间范围继承（非常重要）
用户追问时通常会省略时间范围，必须从对话历史中继承：
- 历史："XX公司2024年全年营业收入是多少？" → 追问："它的毛利率呢？" → 补全为"XX公司2024年全年的毛利率是多少？"
- 历史："XX公司2025年一季度产能利用率是多少？" → 追问："环比提升了多少？" → 补全为"XX公司2025年一季度产能利用率环比提升了多少？"
- 历史："XX公司2024年归属于上市公司股东的净利润是多少？" → 追问："同比下降的主要原因是什么？" → 补全为"XX公司2024年归属于上市公司股东的净利润同比下降的主要原因是什么？"

### 4. 指标/主题继承（多轮追问的核心规则）
多轮对话中，用户可能在不同轮次切换讨论的指标。补全时必须精确判断当前问题在问哪个指标：

**判断原则：先看当前问题本身，再看历史**
- 如果当前问题包含明确的指标词（如"毛利率"、"研发费用"），则只补全该指标对应的公司名和时间范围，不继承历史中其他指标
- 如果当前问题没有明确指标词（如"同比增长了多少？"、"原因是什么？"），则从历史中找到该修饰语最可能修饰的指标

**歧义处理规则（非常重要）：**
当追问的修饰语（如"同比增长"、"下降原因"）可能修饰历史中多个指标时，存在歧义：
- 补全后的问题选择最可能的指标（通常是最近讨论的指标）
- 但 rewritten_query 中必须包含所有可能相关的指标关键词，确保检索不遗漏
- 示例：Q1问"营业收入"，Q2问"毛利率"，Q3问"同比增长了多少？"
  - 补全问题为"XX公司2024年全年的毛利率同比增长了多少？"（最可能指毛利率）
  - rewritten_query 应为"XX公司 2024 全年 毛利率 营业收入 同比 增长 变化"（包含两个指标，确保检索覆盖）

**多轮追问示例：**
- Q1："XX公司2024年全年营业收入是多少？" → Q2："它的毛利率呢？" → 补全为"XX公司2024年全年的毛利率是多少？"（Q2有明确指标"毛利率"，只继承公司名+时间）
- Q1："XX公司2024年全年营业收入是多少？" → Q2："它的毛利率呢？" → Q3："同比增长了多少？" → 补全为"XX公司2024年全年的毛利率同比增长了多少？"（Q3无明确指标，"同比增长"最可能修饰Q2的"毛利率"，但rewritten_query需同时包含"营业收入"和"毛利率"）
- Q1："XX公司2024年全年营业收入是多少？" → Q2："同比增长了多少？" → 补全为"XX公司2024年全年营业收入同比增长了多少？"（Q2无明确指标，"同比增长"修饰Q1的"营业收入"，无歧义）
- Q1："华泰证券将XX公司港股目标价上调至多少？" → Q2："上调的主要理由是什么？" → 补全为"华泰证券将XX公司港股目标价上调的主要理由是什么？"（Q2的"上调"修饰Q1的"目标价上调"）

**禁止行为：**
- 补全后的问题应围绕一个核心指标，不要合并多个指标成一个复合问题
- 但 rewritten_query 作为检索关键词，在存在歧义时应包含所有可能相关的指标词

### 5. 不需要补全的情况
- 如果当前问题已经完整（包含公司名、时间、指标），则只做关键词扩展，不添加历史信息
- 不要添加历史中未提及的新信息

## 关键词扩展规则（非常重要）：
补全问题后，必须进行关键词扩展以提升检索召回率：
- 必须保留问题中的时间范围关键词，如"分季度"、"Q1"、"一季度"、"全年"等
- 必须保留问题中的具体指标关键词，如"营业收入"、"产能利用率"、"净利润"等
- 必须保留问题中的比较关系，如"环比"、"同比"、"分别"等
- 扩展关键词时应添加同义词，但绝对不能替换或删除原始关键词
- 对于"原因"、"为什么"类问题，必须扩展相关术语：原因→因素 动因 由于 因为 影响 驱动
- 对于"变化"、"趋势"类问题，必须扩展相关术语：变化→变动 波动 增减 演变 趋势
- 对于"占比"、"构成"类问题，必须扩展相关术语：占比→比重 构成 结构 比例 份额
- rewritten_query 应包含原始关键词 + 扩展同义词，用空格分隔
...
## 输出格式（严格 JSON）：
{
  ...
  "rewritten_query": "补全并扩展后的关键词（用空格分隔）",
  "doc_type": "年报" | "券商研报" | "调研纪要" | null
}

## 注意：
- 补全后的问题必须能独立理解，不需要依赖对话历史
- 时间范围必须从历史中继承，不能默认为最新或当前时间
- 公司名必须使用历史中的完整名称，不能用简称
- rewritten_query 必须同时包含补全的核心词和扩展的同义词
- doc_type 的判断规则与独立问题相同
"""

HISTORY_REWRITE_HUMAN_TEMPLATE = """以下是之前的对话历史：
{history_summary}

用户的当前问题中可能包含代词、省略的主语或省略的时间范围，请根据对话历史将代词替换为实际实体，将省略的主语和时间范围补充完整，然后进行关键词扩展。
