...
class TestClassifyQuestion:
    ...
    def _make_pipeline(self):
        ...
        pipeline.run_config = RunConfig(rewrite_model="qwen-turbo")
    ...
    @patch("src.api_requests.BaseDashscopeProcessor")
    def test_classify_with_markdown_wrapper(self, MockProcessor):
        """LLM返回被```json```包裹的JSON，应正确解析"""
        mock_instance = MagicMock()
...
class TestRewriteQuestion:
    ...
    def test_rewrite_valid_json(self, MockProcessor):
        """正确解析重写结果"""
        mock_instance = MagicMock()
        mock_instance.send_message.return_value = json.dumps({
    ...
    @patch("src.api_requests.BaseDashscopeProcessor")
    def test_rewrite_with_markdown_wrapper(self, MockProcessor):
        """LLM返回被```包裹的JSON，应正确解析"""
        mock_instance = MagicMock()
